...
void FlightController::follow_carrier(
    float duration_sec,
...
void FlightController::follow_carrier_impl(
    float duration_sec,
    float follow_altitude,
    float forward_offset,
    float left_offset,
...
{
    if (carrier_ == nullptr) {
        RCLCPP_ERROR(logger_, "[follow_carrier]: 未注入小车/航母位姿接口");
        return;
    }
    ...
    RCLCPP_INFO(
        logger_,
        "[follow_carrier]: 开始跟随，duration=%.1fs altitude=%.2fm offset=(forward %.2f, left %.2f)",
        duration_sec,
        follow_altitude,
        forward_offset,
        left_offset);
    ...
    while (rclcpp::ok()) {
        ...
        if (!carrier_->has_recent_carrier_pose(max_pose_age_sec)) {
            ...
            RCLCPP_WARN_THROTTLE(
                logger_, *clock_, 1000,
                "[follow_carrier]: 小车/航母位姿超时，保持当前位置等待 topic 恢复");
            loop_rate.sleep();
            continue;
        }
        ...
        RCLCPP_INFO_THROTTLE(
            logger_, *clock_, 1000,
            "[follow_carrier]: carrier=(%.2f, %.2f, yaw %.2f) target=(%.2f, %.2f, %.2f)",
            carrier.x,
            carrier.y,
            carrier.yaw,
            target.get_x(),
