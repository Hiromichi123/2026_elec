...
DroneHAL::DroneHAL() : Node("drone_hal_node") {
    ...
    const auto carrier_pose_qos = rclcpp::QoS(rclcpp::KeepLast(10)).best_effort().durability_volatile();
...
DroneState DroneHAL::get_carrier_pose() const {
    std::lock_guard<std::mutex> lock(carrier_pose_mutex_);
    return carrier_pose_;
}

bool DroneHAL::has_recent_carrier_pose(double max_age_sec) const {
    std::lock_guard<std::mutex> lock(carrier_pose_mutex_);
    if (!has_carrier_pose_) {
        return false;
    }
...
void DroneHAL::carrier_pose_cb(const ros2_tools::msg::LidarPose::SharedPtr msg) {
    std::lock_guard<std::mutex> lock(carrier_pose_mutex_);
    carrier_pose_.x = msg->x;
    carrier_pose_.y = msg->y;
    carrier_pose_.z = msg->z;
    carrier_pose_.roll = msg->roll;
    carrier_pose_.pitch = msg->pitch;
